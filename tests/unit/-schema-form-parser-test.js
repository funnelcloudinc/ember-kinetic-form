...
module('Unit | Schema Form Parser', function () {
  ...
  test('proxies data from schema', function (assert) {
    ...
    assert.strictEqual(subject.get('type'), 'object');
    assert.strictEqual(subject.get('title'), 'test-title');
  ...
  test('Adds componentName properties using lookup function', function () {
    ...
    sinon.assert.match(subject.get('elements'), [
      sinon.match({
        ...
        type: 'boolean',
        title: 'foo',
        componentName: 'boolean-test-component-name',
      }),
      sinon.match({
        key: '2',
        type: 'string',
        title: 'bar',
        componentName: 'string-test-component-name',
      }),
      sinon.match({
        key: '3',
        type: 'number',
        title: 'baz',
        componentName: 'number-test-component-name',
      }),
    ]);
  });
  ...
  test('parses backwards compatible form data', function () {
    ...
    sinon.assert.match(subject.get('elements'), [
      ...
      sinon.match({
        ...
        type: 'radio',
        title: 'bar',
        componentName: 'radio-test-component-name',
      }),
      sinon.match({
        key: '3',
        type: 'number',
