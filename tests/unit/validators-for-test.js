...
test('returns undefined when fields are not required', function (assert) {
  assert.ok(isNone(validatorsFor({required: false, type: 'string'})), 'expected undefined');
  assert.ok(isNone(validatorsFor({required: false, type: 'boolean'})), 'expected undefined');
  assert.ok(isNone(validatorsFor({required: false, type: 'radios'})), 'expected undefined');
  assert.ok(isNone(validatorsFor({required: false, type: 'number'})), 'expected undefined');
  assert.ok(isNone(validatorsFor({required: false, type: 'textarea'})), 'expected undefined');
});
...
test('returns validators for fields when required', function (assert) {
  ...
  assert.ok(result.every(r => typeOf(r) === 'function'), 'expected an array of functions');
