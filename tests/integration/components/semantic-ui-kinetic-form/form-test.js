...
module(
  'Integration | Component | semantic ui kinetic form/form',
  function (hooks) {
    setupRenderingTest(hooks);

    test('yields a block', async function (assert) {
      await render(
        hbs`{{#semantic-ui-kinetic-form/form}}Test block content{{/semantic-ui-kinetic-form/form}}`
      );
      assert.ok(
    ...
    test('calls onSubmit action when user clicks submit button', async function () {
      ...
      await render(
        hbs`{{semantic-ui-kinetic-form/form onSubmit=(action this.onSubmitSpy)}}`
      );
