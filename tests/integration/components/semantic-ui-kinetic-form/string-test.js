...
moduleForComponent('semantic-ui-kinetic-form/string', 'Integration | Component | semantic ui kinetic form/string', {
  integration: true,
  beforeEach() {
...
test('displays field.title', function(assert) {
  ...
  page.render(hbs`{{semantic-ui-kinetic-form/string field=testField update=(action updateSpy)}}`);
...
test('shows the current value', function(assert) {
  page.render(hbs`{{semantic-ui-kinetic-form/string value="test-value" update=(action updateSpy)}}`);
  assert.equal(page.value, 'test-value');
});

test('calls update action when user enters text', function() {
  page.render(hbs`{{semantic-ui-kinetic-form/string update=(action updateSpy)}}`);
...
test('highlights as an error when error is truthy', function(assert) {
  ...
  page.render(hbs`{{semantic-ui-kinetic-form/string error=testError update=(action updateSpy)}}`);
