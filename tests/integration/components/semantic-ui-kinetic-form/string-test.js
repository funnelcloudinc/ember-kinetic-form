...
import page from '../../../pages/components/semantic-ui-kinetic-form/string';
import sinon from 'sinon';

module('Integration | Component | semantic ui kinetic form/string', function(hooks) {
  ...
  test('displays field.title', async function(assert) {
    ...
    await render(hbs`{{semantic-ui-kinetic-form/string field=this.testField update=(action this.updateSpy)}}`);
  ...
  test('shows the current value', async function(assert) {
    await render(hbs`{{semantic-ui-kinetic-form/string value="test-value" update=(action this.updateSpy)}}`);
    assert.equal(page.value, 'test-value');
  });

  test('calls update action when user enters text', async function() {
    await render(hbs`{{semantic-ui-kinetic-form/string update=(action this.updateSpy)}}`);
  ...
  test('highlights as an error when error is truthy', async function(assert) {
    ...
    await render(hbs`{{semantic-ui-kinetic-form/string error=this.testError update=(action this.updateSpy)}}`);
