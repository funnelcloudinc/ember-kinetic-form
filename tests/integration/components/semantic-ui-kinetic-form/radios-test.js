...
module(
  'Integration | Component | semantic ui kinetic form/radios',
  function (hooks) {
    ...
    test('displays field.title', async function (assert) {
      ...
      await render(
        hbs`{{semantic-ui-kinetic-form/radios field=this.testField update=(action this.updateSpy)}}`
      );
      assert.ok(
    ...
    test('shows the current value', async function (assert) {
      ...
      await render(
        hbs`{{semantic-ui-kinetic-form/radios value=this.testValue update=(action this.updateSpy)}}`
      );
    ...
    test('calls update action when user enters text', async function () {
      await render(
        hbs`{{semantic-ui-kinetic-form/radios update=(action this.updateSpy)}}`
      );
      await page.turnOn();
    ...
    test('highlights as an error when error is truthy', async function (assert) {
      ...
      await render(
        hbs`{{semantic-ui-kinetic-form/radios error=this.testError update=(action this.updateSpy)}}`
      );
