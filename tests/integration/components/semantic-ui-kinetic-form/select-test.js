...
module('Integration | Component | semantic ui kinetic form/select', function(hooks) {
  ...
  test('shows the current value', async function(assert) {
    ...
    await render(
      hbs`{{semantic-ui-kinetic-form/select value="test-value" field=testField update=(action updateSpy)}}`
    );
  ...
  test('highlights as an error when error is truthy', async function(assert) {
    ...
    await render(hbs`{{semantic-ui-kinetic-form/select field=testField error=testError update=(action updateSpy)}}`);
