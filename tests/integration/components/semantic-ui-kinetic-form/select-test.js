...
test('displays field.title', function(assert) {
  ...
  page.render(hbs`{{semantic-ui-kinetic-form/select field=testField update=(action updateSpy)}}`);
...
test('shows the current value', function(assert) {
  ...
  page.render(hbs`{{semantic-ui-kinetic-form/select value="test-value" field=testField update=(action updateSpy)}}`);
...
test('highlights as an error when error is truthy', function(assert) {
  ...
  page.render(hbs`{{semantic-ui-kinetic-form/select error=testError update=(action updateSpy)}}`);
