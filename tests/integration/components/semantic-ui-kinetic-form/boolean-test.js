...
module(
  'Integration | Component | semantic ui kinetic form/boolean',
  function (hooks) {
    ...
    test('displays field.title', async function (assert) {
      ...
      await render(
        hbs`{{semantic-ui-kinetic-form/boolean field=this.testField update=(action this.updateSpy)}}`
      );
      assert.ok(
    ...
    test('shows the current value', async function (assert) {
      await render(
        hbs`{{semantic-ui-kinetic-form/boolean value=true update=(action this.updateSpy)}}`
      );
    ...
    test('calls update action when user enters text', async function () {
      await render(
        hbs`{{semantic-ui-kinetic-form/boolean update=(action this.updateSpy)}}`
      );
      await page.toggle();
    ...
    test('highlights as an error when error is truthy', async function (assert) {
      ...
      await render(
        hbs`{{semantic-ui-kinetic-form/boolean error=this.testError update=(action this.updateSpy)}}`
      );
