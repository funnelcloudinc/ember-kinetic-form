...
test('displays field.title', function(assert) {
  ...
  this.render(hbs`{{semantic-ui-kinetic-form/boolean field=testField update=(action updateSpy)}}`);
...
test('shows the current value', function(assert) {
  this.render(hbs`{{semantic-ui-kinetic-form/boolean value=true update=(action updateSpy)}}`);
...
test('highlights as an error when error is truthy', function(assert) {
  ...
  this.render(hbs`{{semantic-ui-kinetic-form/boolean error=testError update=(action updateSpy)}}`);
