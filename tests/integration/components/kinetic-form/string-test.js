...
test('displays field.title', function(assert) {
  ...
  this.render(hbs`{{kinetic-form/string field=testField update=(action updateSpy)}}`);
...
test('shows the current value', function(assert) {
  this.render(hbs`{{kinetic-form/string value="test-value" update=(action updateSpy)}}`);
  assert.equal(page.value, 'test-value');
});

test('calls update action when user enters text', function() {
  this.render(hbs`{{kinetic-form/string update=(action updateSpy)}}`);
...
test('highlights as an error when error is truthy', function(assert) {
  ...
  this.render(hbs`{{kinetic-form/string error=testError update=(action updateSpy)}}`);
