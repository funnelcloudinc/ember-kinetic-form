...
test('displays field.title', function(assert) {
  ...
  this.render(hbs`{{kinetic-form/radios field=testField update=(action updateSpy)}}`);
...
test('shows the current value', function(assert) {
  ...
  this.render(hbs`{{kinetic-form/radios value=testValue update=(action updateSpy)}}`);
...
test('calls update action when user enters text', function() {
  this.render(hbs`{{kinetic-form/radios update=(action updateSpy)}}`);
  run(() => page.turnOn());
...
test('highlights as an error when error is truthy', function(assert) {
  ...
  this.render(hbs`{{kinetic-form/radios error=testError update=(action updateSpy)}}`);
