...
moduleForComponent('kinetic-form/number', 'Integration | Component | kinetic form/number', {
  integration: true,
  beforeEach() {
...
test('displays field.title', function(assert) {
  ...
  page.render(hbs`{{kinetic-form/number field=testField update=(action updateSpy)}}`);
...
test('shows the current value', function(assert) {
  page.render(hbs`{{kinetic-form/number value=123 update=(action updateSpy)}}`);
  assert.equal(page.value, '123');
});

test('calls update action when user enters text', function() {
  page.render(hbs`{{kinetic-form/number update=(action updateSpy)}}`);
...
test('highlights as an error when error is truthy', function(assert) {
  ...
  page.render(hbs`{{kinetic-form/number error=testError update=(action updateSpy)}}`);
