...
module('Integration | Component | kinetic form/number', function (hooks) {
  ...
  test('displays field.title', async function (assert) {
    ...
    await render(
      hbs`{{kinetic-form/number field=this.testField update=(action this.updateSpy)}}`
    );
    assert.ok(
  ...
  test('shows the current value', async function (assert) {
    await render(
      hbs`{{kinetic-form/number value=123 update=(action this.updateSpy)}}`
    );
  ...
  test('highlights as an error when error is truthy', async function (assert) {
    ...
    await render(
      hbs`{{kinetic-form/number error=this.testError update=(action this.updateSpy)}}`
    );
