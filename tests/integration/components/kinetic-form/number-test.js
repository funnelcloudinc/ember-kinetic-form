...
test('displays field.title', function(assert) {
  ...
  this.render(hbs`{{kinetic-form/number field=testField update=(action updateSpy)}}`);
...
test('shows the current value', function(assert) {
  this.render(hbs`{{kinetic-form/number value=123 update=(action updateSpy)}}`);
...
test('highlights as an error when error is truthy', function(assert) {
  ...
  this.render(hbs`{{kinetic-form/number error=testError update=(action updateSpy)}}`);
