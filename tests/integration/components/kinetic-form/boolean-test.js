...
test('displays field.title', function(assert) {
  ...
  page.render(hbs`{{kinetic-form/boolean field=testField update=(action updateSpy)}}`);
...
test('shows the current value', function(assert) {
  page.render(hbs`{{kinetic-form/boolean value=true update=(action updateSpy)}}`);
  assert.ok(page.isChecked, 'expected checkbox to be checked');
});

test('calls update action when user enters text', function() {
  page.render(hbs`{{kinetic-form/boolean update=(action updateSpy)}}`);
  run(() => page.toggle());
...
test('highlights as an error when error is truthy', function(assert) {
  ...
  page.render(hbs`{{kinetic-form/boolean error=testError update=(action updateSpy)}}`);
