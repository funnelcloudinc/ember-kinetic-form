...
module('Integration | Component | kinetic form/textarea', function (hooks) {
  ...
  test('displays field.title', async function (assert) {
    ...
    await render(
      hbs`{{kinetic-form/textarea field=this.testField update=(action this.updateSpy)}}`
    );
    assert.ok(
  ...
  test('shows the current value', async function (assert) {
    await render(
      hbs`{{kinetic-form/textarea value="test-value" update=(action this.updateSpy)}}`
    );
    assert.ok(
  ...
  test('calls update action when user enters text', async function () {
    await render(hbs`{{kinetic-form/textarea update=(action this.updateSpy)}}`);
  ...
  test('highlights as an error when error is truthy', async function (assert) {
    ...
    await render(
      hbs`{{kinetic-form/textarea error=this.testError update=(action this.updateSpy)}}`
    );
