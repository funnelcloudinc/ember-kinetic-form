...
moduleForComponent('kinetic-form/textarea', 'Integration | Component | kinetic form/textarea', {
  integration: true,
  beforeEach() {
...
test('displays field.title', function(assert) {
  ...
  this.render(hbs`{{kinetic-form/textarea field=testField update=(action updateSpy)}}`);
...
test('shows the current value', function(assert) {
  this.render(hbs`{{kinetic-form/textarea value="test-value" update=(action updateSpy)}}`);
...
test('calls update action when user enters text', function() {
  this.render(hbs`{{kinetic-form/textarea update=(action updateSpy)}}`);
...
test('highlights as an error when error is truthy', function(assert) {
  ...
  this.render(hbs`{{kinetic-form/textarea error=testError update=(action updateSpy)}}`);
